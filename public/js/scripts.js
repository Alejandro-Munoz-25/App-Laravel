...
var url_base = "https://laravel-app-laragram.herokuapp.com/";
...
var page = 1;
var pagelikes = window.location.origin + "/";
var pagehome = window.location.origin + "/likes";
var pageProfiles = window.location.origin + "/profiles";
...
function infiniteScroll() {
  if ($(document).scrollTop() >= $(document).height() - window.innerHeight - 300 && canLoad) {
    canLoad = false;
    ++page;
    loadMoreData(page); // page = (lastPage >= page) ? ++page : 0; //!Scroll infinito
  }
}
...
likes = function likes(element) {
  // var parent = $(element).parent();
  //Comprobacion para cambiar entre like y dislike
  if ($(element).hasClass("like")) {
    $(element).addClass('dislike dark:text-gray-200').removeClass('like fill-current text-red-500'); // $(element).attr('src', url_base + 'images/heart.svg')
    ...
    $(element).addClass('like fill-current text-red-500').removeClass('dislike dark:text-gray-200'); // $(element).attr('src', url_base + 'images/heart_color.svg')
...
openModalComment = function openModalComment(value, id) {
  var modal_overlay_id = document.querySelector('#modal_overlay_' + id);
  var modal_id = document.querySelector('#modal_' + id);
  var modalCl_id = modal_id.classList;
  var overlayCl_id = modal_overlay_id;
  ...
  if (value) {
    ...
    setTimeout(function () {
      return overlayCl_id.classList.add('hidden');
    }, 300);
  }
};
})();
